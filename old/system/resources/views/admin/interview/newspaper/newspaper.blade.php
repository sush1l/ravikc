...
@section('content')
    <div class="header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">पत्रपत्रिका</li>
        </ol>
            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        <form action="{{route('admin.add_newspaper')}} " class="form-horizontal" method="post" enctype="multipart/form-data">
                            @csrf
                        ...
                        <table class="table table-striped table-hover table-responsive" id="dataTables-example">
                            ...
                            @foreach($audio as $audio_sanchar)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{asset('storage/assets/'.$audio_sanchar->file)}}" width="90" alt="{{$audio_sanchar->title}}">
                                    </td>
                                    <td>{{$audio_sanchar->title}}</td>
                                    <td>{!! $audio_sanchar->description !!} </td>
                                    <td>{{$audio_sanchar->u_date}} {{ $audio_sanchar->u_time}}</td>
                                    <td>
                                        <a href="{{route('admin.newspaperedit',[$audio_sanchar->id])}}">
                                            <button class="btn btn-warning btn-sm">
                                                <i class="fa fa-pencil"></i>
                                            </button>
                                        </a>
                                        <a href="{{route('admin.newspaperdelete',[$audio_sanchar->id])}}" onclick="return confirm('Are you sure you want to delete this item?');" title="Delete">
