<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between p-6 sm:py-12 md:px-8 border-b bg-card dark:bg-transparent">
        <div>
            <div class="text-2xl font-extrabold tracking-tight leading-none">पत्रपत्रिका तालिका</div>
        </div>
        <div class="mt-4 sm:mt-0">
            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full">
               <a href="{{ route('admin.mass-communication.newspaper.create') }}"> <i class="fas fa-plus"></i> नयाँ पत्रपत्रिका थप्नुहोस </a>
              </button>
        </div>
    </div>
                                        ...
                                        <tr class="bg-white border-b">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                                ...
                                                <td class="text-sm text-gray-900 font-light py-4 whitespace-nowrap">
                                                    <img class="mx-auto" src="{{ $newspaper->photo }}" alt="Image not loaded" height="150" width="150">
                                                </td>
                                            <td class="text-sm text-gray-900 font-light  py-4 whitespace-nowrap">
                                                {{ $newspaper->title_ne }}
                                            </td>
                                            ...
                                            <td class="text-sm text-gray-900 font-light  py-4 whitespace-nowrap">
                                                {{ $newspaper->upload_date_bs }}
                                            </td>
                                            <td class="flex gap-x-2">
                                                <form action="{{ route('admin.mass-communication.newspaper.destroy',$newspaper) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="my-16 py-1 px-3 bg-red-500 text-white show_confirm"><i class="fas fa-trash"></i></button>
                                                </form>
                                                <a href="{{ route('admin.mass-communication.newspaper.edit',$newspaper) }}">
